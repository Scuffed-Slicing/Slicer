...
public static class SlicerHandler
{
    ...
    {
        double height = 0;
        double maxHeight = ModelHandler.GetMeshHeight(mesh);
     ...
     *
     */
     static PathsD connectPaths(PathsD paths)
    {
    ...
    //bug sometimes random lines get connected
     static PathD Connect(Dictionary<PointD, PathD> connections, PathD path)
    {
     ...
     public static PathsD ErodeAndShell (PathsD slice, double nozzleWidth, int nrShells)
     {
         ...
         PathsD eroded = Clipper.InflatePaths(slice, -nozzleWidth/2, JoinType.Square, EndType.Square);
